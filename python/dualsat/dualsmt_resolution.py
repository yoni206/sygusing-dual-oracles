...
def create_new_variables(cnf_formula, free_vars, proof_size):
...
def setup(path, proof_size):
  proof_size = int(proof_size)
  cnf_formula, free_vars = parse_input_formula(path)
  indicators = create_new_variables(cnf_formula, free_vars, proof_size)
  return path, proof_size, cnf_formula, free_vars, indicators
...
def clause_is_justified_in_resolution_calculus(i):
  assert(i in range(0, proof_size))
  return Or([copy_of_original(i), resolvent(i)])

def is_resolution_proof():
  return And([clause_is_justified_in_resolution_calculus(i) for i in range(proof_size)])
...
def ends_with_bottom():
  return And([Not(indicators[(var, proof_size - 1, polarity)]) for var in free_vars for polarity in [True, False] ])

def is_resolution_proof_of_bottom():
  return And(is_resolution_proof(), ends_with_bottom())
#  return And(is_resolution_proof(), bottom_included())
...
if __name__ == "__main__":
  path, proof_size, cnf_formula, free_vars, indicators = setup(sys.argv[1], sys.argv[2])
  
  #compute the dual formula
  result = is_resolution_proof_of_bottom()
